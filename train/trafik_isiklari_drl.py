...
class TrafikIsigiOrtami(gym.Env):
    ...
    def __init__(self, gui=False, max_adim=3600, render_mode=None):
        ...
        self.kavsaklar = ["J1"]  # Kontrol edilecek kavşak ID'leri
    ...
    def _sumo_baslat(self):
        ...
        try:
            ...
            if self.gui:
                ...
                sumo_cmd = [sumo_binary, "-c", os.path.join(sumo_files_dir, "sumo.sumocfg"), "--start", "--quit-on-end"]
            else:
                sumo_binary = "sumo"
                sumo_cmd = [sumo_binary, "-c", os.path.join(sumo_files_dir, "sumo.sumocfg"), "--no-step-log", "--no-warnings"]
    ...
    def _gozlem_al(self) -> np.ndarray:
        ...
        mevcut_faz = traci.trafficlight.getPhase("J1") # Sadece J1 kavşağı için
        faz_suresi = traci.trafficlight.getPhaseDuration("J1")
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        # Aksiyonu uygula
        if action == 1:  # Fazı değiştir
            traci.trafficlight.setPhase("J1", (traci.trafficlight.getPhase("J1") + 1) % 4) # 4 faz olduğunu varsayalım
